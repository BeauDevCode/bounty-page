...
# Do not derive these values from /app/access.log at verification time because
# an agent can modify files under /app and make a bad report self-consistent.
...
def test_success_criterion_1_report_schema() -> None:
    """Success criterion 1: the report exists and has the exact JSON schema and types."""
...
def test_success_criterion_2_total_requests() -> None:
    """Success criterion 2: total_requests matches the shipped access log."""
    report = load_report()
    assert report["total_requests"] == EXPECTED_TOTAL_REQUESTS


def test_success_criterion_3_unique_ips() -> None:
    """Success criterion 3: unique_ips matches the shipped access log."""
    report = load_report()
    assert report["unique_ips"] == EXPECTED_UNIQUE_IPS


def test_success_criterion_4_top_path() -> None:
    """Success criterion 4: top_path matches the shipped access log."""
    report = load_report()
