...
Analyze the log and write a report to `/app/report.json`. The report must be one JSON object containing exactly these three keys:
...
- `top_path` (string): the request path that appears most frequently. The path is the value between the HTTP method and HTTP version inside the quoted request. The shipped log has one unique most-frequent path, so no tie-breaking rule is needed.
